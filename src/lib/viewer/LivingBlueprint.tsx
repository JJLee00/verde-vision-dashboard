...
export function LivingBlueprint({
  ...
  useEffect(() => {
    ...
    let raf = 0;
    const frame = () => {
      if (embed && !reducedMotion) {
        cur.az += 0.05; // slow ambient orbit for the preview card
        tgt.az = cur.az;
      }
  ...
  const canvasPane = (
    <div
      ref={wrapRef}
      className={embed ? "relative h-full w-full" : "relative min-h-0 flex-1"}
    >
      <canvas
        ref={canvasRef}
